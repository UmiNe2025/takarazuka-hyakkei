...
> 困ったとき・調べたいときに「まずここを開けば、次にすることがわかる」生活ポータル。

市公式サイトは情報が深い階層に分かれていて、目的のページに辿り着くまでが大変。
本サイトは **「入口の地図」** に徹する — 各テーマの要点（連絡先・時間・費用・手順）を一覧化し、正確な詳細は必ず市公式ページへリンクで誘導する。トップでは、住民が実際に直面する「救急」「引越し」「今日のごみ」「子ども」「防災」「相談」の6場面から、最初の一手へ直接進める。
...
8. **個人化は端末内だけ** — 各案内を「あとで見る」に保存できる。保存先はブラウザの `localStorage` のみで、アカウント・計測・外部送信は行わない。
9. **生活の机** — 地域と子どもの年齢を任意で選ぶ「わたしの宝塚、今日の机」をトップに置く。保存するのは地域区分・年齢区分のみで、住所・集合場所・連絡先は保存しない。
...
トップで毎回使う小さな判断をまとめる。地域の割り当ては厳密な住所判定ではなく、誤案内を避けるため**広域の目安**として扱い、町名単位で担当が異なる情報は公式案内へのリンクを併記する。

| 機能 | 役立つ場面 | 保存・データの扱い |
|---|---|---|
| 子どもと今週 | 親子向け・年齢に近い市イベントを探す | 市イベントの更新データを読込み。年齢区分は端末内だけに保存 |
| 親の介護をはじめる | 相談・認定・退院前連携の順序を知る | 地域包括支援センターは広域の目安。電話と公式案内を表示 |
| マイ地区・今日の宝塚 | 近隣候補の避難所、地域名を含む直近イベントを確認 | 地域区分のみ端末内に保存。避難所の開設状況は市の発表を優先 |
| わが家の防災カード | 集合場所・連絡方法を書いて紙で持つ | 入力内容は保存せず、ブラウザ印刷だけを行う |

### トップの「困りごとから、次にすることへ」

カテゴリを知っている前提にせず、複数の窓口が必要になりやすい生活場面を6つの行動導線として配置する。

| 場面 | 最初の一手 | 続けて確認すること |
|---|---|---|
| 救急の判断 | #7119（命の危険なら119） | 夜間・休日の診療先 |
| 引越し | 転入・転出・転居 | 水道、粗大ごみ |
| 今日のごみ | 分別・収集日の確認 | 粗大ごみ・持込み |
| 子どもの急病 | #8000 | 子育て支援、休日・夜間診療 |
| 災害への備え | ハザードマップ・避難所 | 災害時の連絡先 |
| 生活の相談 | 消費生活センター・188 | 福祉・無料法律相談 |
...
├── life.js                 ← 横断検索・絞り込み・端末内「あとで見る」・生活の机（プログレッシブ強化／JSなしでも全文閲覧可）
├── assets/                 ← hero.png / ogp.png
...
- トップは「困りごとから、次にすることへ」の6導線を主役にし、カテゴリ一覧はその次に置く
- よく使う案内は「あとで見る」に保存可能（この端末だけ・外部送信なし）
