...
## セッション追記(第2弾: 2026-07-06)「日常を楽しむ」ギャップ埋め
- **新ガイド③ 年間イベント・歳時記カレンダー** (初詣〜梅〜桜〜花火〜宝塚記念〜ダリア〜紅葉。年変動日付は「例年」+公式確認で誠実に)。(commit f563c5c)
- **新ガイド④ 季節の花ガイド** (中山寺梅1000本/花のみち桜/桜の園 笹部新太郎30種5000本/ダリア園300種10万本・花摘み100円)。(commit f563c5c)
- **新ガイド⑤ 子ども・家族で楽しむ おでかけガイド** (無料スポット/雨の日/花摘み/ベビーカー・バリアフリー、くらしの便利帳へ相互リンク)。(commit a551da9)
- **重要な誠実性の発見**: 生活側 disaster カテゴリは既に7項目(ハザードマップ4種解説・避難所・給水拠点・耐震補助等)で充実しており、競合分析の「防災ギャップ」は概ね既存だった。→ 冗長な防災ページ追加をやめ、真に不足の「家族の日常の楽しみ」へ方針転換(生成器/guide.jsonの高リスク改変も回避)。
...
3. 観光: 西谷の里山体験、宝塚温泉、小浜宿さんぽ 等の個別ガイド(research R2-R4に素材あり)。
4. 技術(低・要検討): dark mode / 百景個別URL化 / SVGのImageObject化・EN aria-label / sitemap lastmod ファイル別。
- フォント非同期化は現CSP(script-src厳格)が inline onload を弾くため保留(preload方式 or CSP調整が前提)。
